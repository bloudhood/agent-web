...
function createConfigManager(configDir, deps) {
  ...
  const CC_SWITCH_MANAGED_APPS = ['claude', 'codex', 'gemini'];
  ...
  function ccSwitchAppLabel(app) {
    return app === 'claude' ? 'Claude' : app === 'codex' ? 'Codex' : app === 'gemini' ? 'Gemini' : app;
  }

  function getCcSwitchAppState(app, toolStatus = null) {
  ...
  function getCcSwitchState() {
    ...
    return {
      cli,
      toolStatus,
      apps: Object.fromEntries(CC_SWITCH_MANAGED_APPS.map((app) => [app, getCcSwitchAppState(app, toolStatus)])),
    };
  }
