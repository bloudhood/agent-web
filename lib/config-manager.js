...
const { createHermesProviderBridge } = require('./hermes-provider-bridge');
 ...
 * @param {Function} [deps.buildProcessLaunch] – normalizes Windows .cmd/.ps1 launch
...
function createConfigManager(configDir, deps) {
  ...
  const CC_SWITCH_MANAGED_APPS = ['claude', 'codex', 'gemini', 'hermes'];
  ...
  function readCodexLocalConfigSnapshot() {
    ...
    const config = { apiKey: '', apiBase: '', model: '', provider: '' };
    ...
    try {
      if (fs.existsSync(codexConfigToml)) {
        ...
        Object.assign(config, parseCodexConfigToml(toml));
      }
    } catch {}
  ...
  function parseCodexConfigToml(toml) {
    ...
    for (const line of String(toml || '').split(/\r?\n/)) {
      ...
      const value = parseTomlString(keyMatch[2]);
      if (!currentSection) {
        if (key === 'model') config.model = value;
        if (key === 'model_provider') config.provider = value;
        continue;
      }

      const providerId = codexProviderSectionId(currentSection);
      if (!providerId) continue;
      const provider = providers.get(providerId) || {};
      if (key === 'base_url') provider.apiBase = value;
      providers.set(providerId, provider);
    }

    if (config.provider && providers.get(config.provider)?.apiBase) {
      config.apiBase = providers.get(config.provider).apiBase;
    } else if (providers.size > 0) {
      config.apiBase = Array.from(providers.values()).find((provider) => provider.apiBase)?.apiBase || '';
    }
  ...
  function describeCodexRuntimeSource() {
    const codexConfig = loadCodexConfig();
    if (codexConfig.mode === 'custom') {
      ...
      return {
        mode: 'custom',
        source: 'agent-web custom profile',
        profile: activeProfile?.name || codexConfig.activeProfile || '',
        provider: activeProfile ? 'openai_compat' : '',
        apiBase: activeProfile ? normalizeCodexRuntimeApiBase(activeProfile.apiBase) : '',
        model: activeProfile?.model || DEFAULT_CODEX_MODEL,
        auth: activeProfile?.apiKey ? 'agent-web profile API key' : 'missing API key',
        sourceFound: !!activeProfile,
      };
    }

    const snapshot = readCodexLocalConfigSnapshot();
    return {
      mode: 'local',
      source: snapshot.sourceFound ? 'local ~/.codex config' : 'local ~/.codex config not found',
      profile: '',
      provider: snapshot.config.provider || '',
      apiBase: snapshot.config.apiBase || '',
      model: snapshot.config.model || DEFAULT_CODEX_MODEL,
      auth: snapshot.hasApiKey ? '~/.codex/auth.json' : 'native login or missing API key',
      sourceFound: snapshot.sourceFound,
    };
  }
  ...
  function parseTomlString(rawValue) {
    const raw = String(rawValue || '').trim().replace(/\s+#.*$/, '');
    if (!raw) return '';
    try {
  ...
  function providerSectionMatches(sectionName, providerName) {
    if (!sectionName || !providerName) return false;
    const normalized = sectionName.toLowerCase();
    const provider = String(providerName).trim();
    return normalized === `model_providers.${provider.toLowerCase()}`
      || normalized === `model_providers."${provider.toLowerCase()}"`
      || normalized === `model_providers.'${provider.toLowerCase()}'`;
  }

  function extractCodexLocalRuntimeConfig(sourceText) {
    ...
    const safeProviderKeys = new Set(['name', 'base_url', 'env_key', 'wire_api', 'requires_openai_auth']);
    const lines = [];
    ...
    for (const rawLine of sourceLines) {
      ...
      const nextSection = tomlSectionName(trimmed);
      if (nextSection) {
        section = nextSection;
        continue;
      }
      ...
      if (!match) continue;
      const key = match[1];
      if (!keepTopLevelKeys.has(key)) continue;
      lines.push(trimmed);
      if (key === 'model_provider') activeProvider = parseTomlString(match[2]);
    }
    ...
    if (activeProvider) {
      ...
      for (const rawLine of sourceLines) {
        ...
        if (!providerSectionMatches(section, activeProvider) || !trimmed || trimmed.startsWith('#')) continue;
        const match = trimmed.match(/^([A-Za-z0-9_]+)\s*=/);
        if (match && safeProviderKeys.has(match[1])) providerLines.push(trimmed);
      }
      if (providerLines.length > 0) {
        lines.push('', `[model_providers.${activeProvider}]`, ...providerLines);
      }
    }
  ...
  function prepareCodexLocalRuntimeHome(homeDir) {
    ...
    const lines = extractCodexLocalRuntimeConfig(sourceText);
  ...
  function ccSwitchLaunch(candidate, args) {
  ...
  function findCcSwitchCli() {
    ...
    for (const candidate of ccSwitchCandidatePaths()) {
      ...
      try {
        const launch = ccSwitchLaunch(candidate, ['--version']);
  ...
  function runCcSwitch(args, timeout = 15000) {
    ...
    try {
      const launch = ccSwitchLaunch(cli.path, args);
      const result = spawnSync(launch.command, launch.args, {
        encoding: 'utf8',
        timeout,
        windowsHide: true,
  ...
  function parseCcSwitchTools(output) {
    const tools = {};
    const keyMap = {
      claude: 'claude',
      codex: 'codex',
    ...
    for (const line of String(output || '').split(/\r?\n/)) {
      ...
      if (!label || /^Tool$/i.test(label) || !status || /^Status$/i.test(status)) continue;
      const compact = label.toLowerCase().replace(/\s+/g, '');
      const key = keyMap[compact] || keyMap[label.toLowerCase()] || compact;
      const versionMatch = status.match(/\(([^)]+)\)/);
      tools[key] = {
        ok: /^ok\b/i.test(status),
        label,
        status,
  ...
  function getCcSwitchEnvStatus(app) {
    const result = runCcSwitch(['--app', app, 'env', 'check']);
    const output = `${result.stdout || ''}\n${result.stderr || ''}`.trim();
    const hasNoConflict = /No environment variable conflicts detected/i.test(output);
    return {
      ok: !!result.ok && (hasNoConflict || !/conflict detected|conflicts detected|error/i.test(output)),
      app,
      summary: firstMeaningfulLine(output) || (result.ok ? '环境检查通过' : '环境检查失败'),
      output: output.split(/\r?\n/).slice(0, 8).join('\n'),
      error: result.ok ? '' : (result.error || result.stderr || result.stdout || `cc-switch env check failed for ${app}`),
    };
  }

  function getCcSwitchToolStatus() {
    const result = runCcSwitch(['env', 'tools']);
    if (!result.ok) {
      const message = result.error || result.stderr || result.stdout || 'cc-switch env tools failed';
      return { ok: false, error: message.split(/\r?\n/).slice(0, 3).join('\n'), tools: {} };
    }
    return { ok: true, tools: parseCcSwitchTools(result.stdout) };
  }

  const hermesProviderBridge = createHermesProviderBridge({ isWindows: IS_WIN, env: process.env, firstMeaningfulLine });
  function getHermesSwitchAppState(options = {}) { return hermesProviderBridge.getState(options); }

  function ccSwitchManagedCliCommand(app) {
    if (app === 'claude') return CLAUDE_PATH;
    if (app === 'codex') return CODEX_PATH;
    if (app === 'gemini') return GEMINI_PATH;
    return app;
  }

  function getLocalCliToolFallback(app) {
    if (typeof buildProcessLaunch !== 'function') return null;
    const command = ccSwitchManagedCliCommand(app);
    try {
      ...
      const output = sanitizeCcSwitchOutput(`${result.stdout || ''}${result.stderr || ''}`);
      const version = firstMeaningfulLine(output);
      if (result.status === 0 && version) {
        return {
          ok: true,
          label: ccSwitchAppLabel(app),
          status: `ok (${version})`,
          version,
          source: 'local-version-fallback',
        };
      }
      return {
        ok: false,
        label: ccSwitchAppLabel(app),
        status: version || `error (${result.status ?? 'unknown'})`,
        version: '',
        source: 'local-version-fallback',
      };
    } catch (err) {
      return {
        ok: false,
        label: ccSwitchAppLabel(app),
        status: `error (${err.message || 'program not found'})`,
        version: '',
        source: 'local-version-fallback',
      };
    }
  ...
  function normalizeCcSwitchApp(app) {
    ...
    return CC_SWITCH_MANAGED_APPS.includes(value) ? value : '';
  }

  function ccSwitchAppLabel(app) {
    return app === 'claude' ? 'Claude' : app === 'codex' ? 'Codex' : app === 'gemini' ? 'Gemini' : app === 'hermes' ? 'Hermes' : app;
  }

  function getCcSwitchAppState(app, toolStatus = null, options = {}) {
    if (app === 'hermes') return getHermesSwitchAppState(options);
    const result = runCcSwitch(['--app', app, 'provider', 'list']);
    const rawTool = toolStatus?.tools?.[app] || null;
    const fallbackTool = rawTool?.ok ? null : getLocalCliToolFallback(app);
    const tool = fallbackTool?.ok ? { ...fallbackTool, ccSwitchStatus: rawTool?.status || '' } : rawTool;
    const envStatus = getCcSwitchEnvStatus(app);
    if (!result.ok) {
      const message = result.error || result.stderr || result.stdout || `cc-switch provider list failed for ${app}`;
      return { ok: false, app, error: message.split(/\r?\n/).slice(0, 3).join('\n'), providers: [], envStatus, toolStatus: tool };
    }
    ...
    return {
      ...
      currentProviderApiUrl: current?.apiUrl || '',
      envStatus,
      toolStatus: tool,
    };
  }

  function getCcSwitchState(options = {}) {
    const cli = findCcSwitchCli();
    if (!cli.ok) return { cli, apps: { hermes: getHermesSwitchAppState(options) } };
    const toolStatus = getCcSwitchToolStatus();
    return {
      cli,
      toolStatus,
      apps: Object.fromEntries(CC_SWITCH_MANAGED_APPS.map((app) => [app, getCcSwitchAppState(app, toolStatus, options)])),
    };
  }
  ...
  function handleSaveCodexConfig(ws, newConfig) {
    ...
    wsSend(ws, {
      ...
      message: requestedSearch
        ? 'Codex 配置已保存。当前 agent-web 的 Codex exec 路径暂未接入 Web Search，已自动忽略该开关。'
        : 'Codex 配置已保存',
    });
  }

  function handleGetCcSwitchState(ws, msg = {}) {
    wsSend(ws, { type: 'ccswitch_state', state: getCcSwitchState({ forceRefresh: !!msg.forceRefresh }) });
  }
  ...
  function handleSwitchCcSwitchProvider(ws, msg) {
    ...
    if (app === 'hermes') {
      const result = hermesProviderBridge.switchProviderWithState(providerId);
      if (!result.ok) return wsSend(ws, { type: 'ccswitch_switch_result', success: false, app, providerId, message: result.message });
      plog('INFO', 'hermes_provider_switched', { providerId, providerName: result.providerName || null });
      wsSend(ws, { type: 'ccswitch_switch_result', success: true, app, providerId, providerName: result.providerName, message: `Hermes 已切换到 ${result.providerName}` });
      wsSend(ws, { type: 'ccswitch_state', state: getCcSwitchState() });
      wsSend(ws, { type: 'system_message', message: `Hermes Provider 已切换为 ${result.providerName}` });
    ...
    const appLabel = ccSwitchAppLabel(app);
    wsSend(ws, {
      type: 'ccswitch_switch_result',
      success: true,
      app,
      ...
      message: `${appLabel} 已切换到 ${target.name || providerId}${desktopRefresh.ok ? '，桌面端已刷新' : '，但桌面端刷新失败'}`,
      desktopRefresh,
    });
    wsSend(ws, { type: 'ccswitch_state', state: getCcSwitchState() });
    wsSend(ws, { type: 'system_message', message: `${appLabel} Provider 已切换为 ${target.name || providerId}` });
    if (sendSessionList) sendSessionList(ws);
  }
  ...
  return {
    ...
    // CC Switch
    getCcSwitchState,
    resolveHermesModelLabel: hermesProviderBridge.resolveModelLabel,
