...
function createSessionStore(sessionsDir, deps) {
  ...
  function normalizePermissionModeForAgent(agent, mode) {
    ...
    if (normalizedAgent === 'hermes') return 'yolo';
  ...
  function permissionModeEffect(agent, mode) {
    const normalizedAgent = normalizeAgent(agent);
    const normalizedMode = normalizePermissionModeForAgent(normalizedAgent, mode);
    if (normalizedAgent === 'codex') {
      if (normalizedMode === 'yolo') return '实际生效: codex exec --dangerously-bypass-approvals-and-sandbox';
      if (normalizedMode === 'plan') return '实际生效: codex exec -s read-only';
      return '实际生效: codex exec --full-auto';
    }
    if (normalizedAgent === 'gemini') {
      return `实际生效: gemini --approval-mode ${normalizedMode}`;
    }
    if (normalizedAgent === 'hermes') {
      return '实际生效: Hermes API 不接收本地 CLI 权限参数，仅记录 agent-web 会话模式';
    }
    if (normalizedMode === 'yolo') return '实际生效: claude --dangerously-skip-permissions';
    return `实际生效: claude --permission-mode ${normalizedMode}`;
  }
  ...
  function handleNewSession(ws, msg) {
    const { MODEL_MAP, resolveDefaultCodexModel, loadDevConfig, handleMessage, CONFIG_DIR, sessionModelLabel } = lateBound;
    ...
    let resolvedCwd = cwd || defaultWorkspaceForAgent(agent);
    let hostInfo = null;
    ...
    const modelLabel = typeof sessionModelLabel === 'function'
      ? sessionModelLabel(session)
      : (agent === 'hermes' ? (session.model || 'Hermes')
  ...
  function handleSetMode(ws, sessionId, mode) {
    ...
    if (sessionId) {
      ...
      if (session) {
        ...
        session.updated = new Date().toISOString();
        saveSession(session);
        wsSend(ws, { type: 'system_message', message: permissionModeEffect(agent, session.permissionMode) });
        wsSend(ws, { type: 'mode_changed', mode: session.permissionMode });
        return;
      }
    }
    const normalizedMode = normalizePermissionModeForAgent('claude', mode);
    wsSend(ws, { type: 'system_message', message: permissionModeEffect('claude', normalizedMode) });
  ...
  function handleListGeminiSessions(ws) {
    wsSend(ws, { type: 'gemini_sessions', sessions: listGeminiSessions ? listGeminiSessions() : [] });
  }

  function handleImportGeminiSession(ws, msg) {
    const geminiSessionId = String(msg?.sessionId || '').trim();
    if (!geminiSessionId) return wsSend(ws, { type: 'error', message: '缺少 sessionId' });
    const parsed = parseGeminiChatFile ? parseGeminiChatFile(msg?.chatPath || '') : null;
    if (!parsed || parsed.meta.sessionId !== geminiSessionId) {
      return wsSend(ws, { type: 'error', message: '未找到对应的 Gemini 会话文件' });
    }
    ...
    try {
      for (const f of fs.readdirSync(sessionsDir).filter(f => f.endsWith('.json'))) {
        try {
          const s = normalizeSession(JSON.parse(fs.readFileSync(path.join(sessionsDir, f), 'utf8')));
          if (s.geminiSessionId === geminiSessionId) { existingSession = s; break; }
        } catch {}
      }
    } catch {}
    ...
    const session = {
      id,
      title: parsed.meta.title || existingSession?.title || geminiSessionId.slice(0, 20),
      created: existingSession?.created || new Date().toISOString(),
      updated: new Date().toISOString(),
      agent: 'gemini',
      claudeSessionId: null,
      ...
      geminiSessionId,
      importedFrom: 'gemini',
      importedGeminiChatPath: parsed.filePath,
