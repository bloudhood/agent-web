  ...
  function deleteCodexLocalSession(session) {
    ...
    try {
      ...
      if (sqliteCheck.status !== 0) {
        ...
        plog('WARN', 'codex_delete_no_sqlite3', diag);
        process.stderr.write(`[codex-delete] sqlite3 not found: ${JSON.stringify(diag)}\n`);
      } else {
        ...
        const dbExists = fs.existsSync(CODEX_STATE_DB_PATH);
        ...
        if (stateResult.status === 0) {
          ...
          plog('WARN', 'codex_delete_state_db_failed', diag);
          process.stderr.write(`[codex-delete] state db delete failed: ${JSON.stringify(diag)}\n`);
        }
        ...
        if (fs.existsSync(CODEX_LOG_DB_PATH)) {
          ...
          if (logResult.status !== 0) {
            ...
            plog('WARN', 'codex_delete_log_db_failed', diag);
            process.stderr.write(`[codex-delete] log db delete failed: ${JSON.stringify(diag)}\n`);
          }
        }
      }
    } catch (err) {
      process.stderr.write(`[codex-delete] exception: ${err.message || err}\n`);
    }
