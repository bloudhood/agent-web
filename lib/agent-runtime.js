function createAgentRuntime(deps) {
  ...
  function sendSessionEvent(entry, sessionId, payload, skipIfClosed = false) {
    wsSend(entry.ws, { ...payload, sessionId }, skipIfClosed);
  }

  function completeTurn(entry, sessionId, reason) {
    if (entry.turnFinalized || typeof onTurnComplete !== 'function') return;
    onTurnComplete(sessionId, entry, reason);
  }
    ...
    switch (event.type) {
      ...
      case 'assistant': {
        ...
        for (const block of content) {
          if (block.type === 'text' && block.text) {
            ...
            sendSessionEvent(entry, sessionId, { type: 'text_delta', text: block.text }, true);
            ...
            sendSessionEvent(entry, sessionId, { type: 'tool_start', name: block.name, toolUseId: block.id, input: tc.input });
            ...
            sendSessionEvent(entry, sessionId, { type: 'tool_end', toolUseId: block.tool_use_id, result: resultText.slice(0, 2000) });
          }
        }
      ...
      case 'result': {
        ...
        if (entry.ws && event.total_cost_usd !== undefined) {
          sendSessionEvent(entry, sessionId, { type: 'cost', costUsd: session?.totalCost || 0 }, true);
        }
        completeTurn(entry, sessionId, 'claude_result');
        break;
      }
    }
  }
    ...
    switch (event.type) {
      ...
      case 'item.completed': {
        ...
        if (item.type === 'agent_message') {
          if (item.text) {
            ...
            sendSessionEvent(entry, sessionId, { type: 'text_delta', text: item.text }, true);
          }
          break;
        }
      ...
      case 'turn.completed': {
        ...
        if (session && usage) {
          ...
          saveSession(session);
          sendSessionEvent(entry, sessionId, { type: 'usage', totalUsage: session.totalUsage }, true);
        }
        completeTurn(entry, sessionId, 'codex_turn_completed');
        break;
      }
  ...
  function ensureHermesToolCall(entry, item, sessionId) {
    ...
    switch (eventType) {
      ...
      case 'response.output_text.delta': {
        ...
        if (delta) {
          ...
          sendSessionEvent(entry, sessionId, { type: 'text_delta', text: delta }, true);
        }
        break;
      }
      ...
      case 'response.output_item.added': {
        ...
        if (item.type === 'function_call') ensureHermesToolCall(entry, item, sessionId);
        break;
      }
      ...
      case 'response.output_item.done': {
        ...
        if (item.type === 'function_call') {
          const tc = ensureHermesToolCall(entry, item, sessionId);
          if (tc) {
      ...
      case 'response.completed': {
        ...
        if (!entry.fullText) {
          ...
          if (finalText) {
            ...
            sendSessionEvent(entry, sessionId, { type: 'text_delta', text: finalText }, true);
          }
        }
        completeTurn(entry, sessionId, 'hermes_response_completed');
        break;
      }
  ...
  function ensureGeminiToolCall(entry, event, sessionId) {
    ...
    switch (event.type) {
      ...
      case 'message': {
        ...
        if (text) {
          ...
          sendSessionEvent(entry, sessionId, { type: 'text_delta', text }, true);
        }
        break;
      }

      case 'tool_use':
        ensureGeminiToolCall(entry, event, sessionId);
        break;
      ...
      case 'result': {
        ...
        if (event.status !== 'error') completeTurn(entry, sessionId, 'gemini_result');
        break;
      }
    }
  }
