...
const { buildHermesResponseInput } = require('./hermes-response-input');
const { createRunPaths, listRunEntries } = require('./run-files');
...
function createAgentManager(deps) {
  ...
  function cleanRunDir(sessionId, targetDir = null) {
    const dir = targetDir || runDir(sessionId);
    try {
      if (targetDir) fs.rmSync(path.join(dir, 'pid'), { force: true });
      if (fs.existsSync(dir)) fs.rmSync(dir, { recursive: true });
    } catch {}
  }

  function cleanupRunPath(sessionId, targetDir) {
    cleanRunDir(sessionId, targetDir);
    const baseDir = runDir(sessionId);
    if (!targetDir || path.resolve(targetDir) === path.resolve(baseDir)) return;
    try {
  ...
  function formatRuntimeError(agent, raw, context = {}) {
    ...
    if (agent === 'gemini') {
      ...
      if (/trusted directory|trust.*workspace|skip-trust/i.test(condensed)) {
        return 'Gemini CLI 拒绝运行未信任工作区。agent-web 已使用 --skip-trust；若仍失败，请检查 Gemini CLI 的 trust 设置。';
      }
    ...
    if (agent === 'codex') {
      ...
      if (/unexpected argument|unexpected option|Usage:\s*codex/i.test(raw || '')) {
        return `Codex CLI 参数不兼容：${firstMeaningfulLine(condensed)}。建议检查当前 CLI 版本与 agent-web 的参数约定是否匹配。`;
      }
  ...
  function compactStartMessage(agent) {
    if (agent === 'gemini') return 'Gemini 会话暂不支持通过 agent-web 执行 /compact。';
    return agent === 'codex'
  ...
  function buildCodexInitPrompt(cwd) {
    ...
    return [
      'You are running agent-web\'s /init for a Codex session.',
  ...
  function defaultWorkspaceForAgent(agent) {
    return normalizeAgent(agent) === 'hermes'
      ? null
  ...
  function handleTurnComplete(sessionId, entry, reason) {
    const activeEntry = activeProcesses.get(sessionId);
    if (!activeEntry || activeEntry !== entry || entry.turnFinalized) return;
    ...
    plog('INFO', 'logical_turn_complete', {
    ...
    handleProcessComplete(sessionId, 0, null, entry.pid ?? null);
  }

  function handleProcessComplete(sessionId, exitCode, signal, expectedPid = null) {
    const entry = activeProcesses.get(sessionId);
    if (!entry) return;
    if (expectedPid !== null && entry.pid !== expectedPid) {
      plog('INFO', 'process_complete_stale_ignored', {
        sessionId: sessionId.slice(0, 8),
        expectedPid,
        activePid: entry.pid,
      });
    ...
    const wsShowsSession = !!entry.ws && wsSessionMap.get(entry.ws) === sessionId;
    ...
    let stderrSnippet = '';
    try {
      const errPath = entry.errorPath || path.join(entry.runDir || runDir(sessionId), 'error.log');
    ...
    cleanupRunPath(sessionId, entry.runDir);
  ...
  function recoverProcesses() {
    try {
      const entries = listRunEntries(SESSIONS_DIR);
      if (entries.length === 0) return;
      plog('INFO', 'recovery_start', { runDirs: entries.length });
      for (const run of entries) {
        const { sessionId, dir, pidPath, outputPath, errorPath } = run;
        const session = loadSession(sessionId);
        ...
        if (isProcessRunning(pid)) {
          ...
          const entry = { pid, ws: null, agent, fullText: '', toolCalls: [], lastCost: null, lastUsage: null, lastError: null, errorSent: false, runDir: dir, outputPath, errorPath, tailer: null };
  ...
  async function startHermesResponseStream(ws, session, textValue, attachments = []) {
    ...
    let responseInput;
    try {
      responseInput = buildHermesResponseInput(textValue, attachments);
    } catch (err) {
      wsSend(ws, { type: 'error', message: `Hermes 图片附件读取失败：${err.message}` });
      return false;
    }
    const conversationId = hermesConversationIdForSession(session);
    session.hermesConversationId = conversationId; session.updated = new Date().toISOString(); saveSession(session);
    ...
    activeProcesses.set(sessionId, entry);
    wsSend(ws, { type: 'model_changed', model: sessionModelLabel(session) || 'Hermes' });
    sendSessionList(ws);
  ...
  function handleMessage(ws, msg, options = {}) {
    ...
    if (!session) {
      ...
      const resolvedCwd = defaultWorkspaceForAgent(agent);
      session = {
        id,
        title: derivedTitle,
    ...
    if (isHermesSession(session)) {
      startHermesResponseStream(ws, session, textValue, resolvedAttachments);
      return;
    }
    ...
    const { dir, inputPath, outputPath, errorPath, pidPath } = createRunPaths(runDir(currentSessionId));
    ...
    try {
      ...
      errorFd = fs.openSync(errorPath, 'w');
    } catch (err) {
      if (typeof outputFd === 'number') try { fs.closeSync(outputFd); } catch {}
      if (typeof errorFd === 'number') try { fs.closeSync(errorFd); } catch {}
      cleanupRunPath(currentSessionId, dir);
      plog('ERROR', 'process_io_prepare_fail', {
    ...
    proc.on('exit', (code, signal) => {
      ...
      if (entry && entry.pid === proc.pid) entry.exitObserved = { code, signal };
      ...
      setTimeout(() => {
        ...
        const observed = latest?.pid === proc.pid && latest.exitObserved ? latest.exitObserved : { code, signal };
        handleProcessComplete(currentSessionId, observed.code, observed.signal, proc.pid);
      }, 300);
    });
    ...
    proc.on('error', (err) => {
      ...
      setTimeout(() => handleProcessComplete(currentSessionId, -1, null, proc?.pid || null), 300);
    });
