...
function createRouter(deps) {
  ...
  function availableModesForAgent(agent) {
    return agent === 'gemini' ? ['plan', 'yolo'] : ['default', 'plan', 'yolo'];
  }
  ...
  function buildSlashCompletions(agent, input) {
    ...
    if (!tokens.length || tokens[0] === '/' || (tokens.length === 1 && !trailingSpace)) {
      const native = nativeTopLevelCompletions(normalizedAgent, input);
      const nativeCommands = new Set(native.map((command) => command.cmd));
      const web = webSlashCompletions(normalizedAgent, input)
        .filter((command) => !nativeCommands.has(command.cmd));
      return [...native, ...web].slice(0, 80);
    }
